...
-- Внутрішня бібліотека Platoboost (JSON & Crypto)
...
function copyLink()
    ...
    if response.StatusCode == 200 then
        ...
        if decoded.success then fSetClipboard(decoded.data.url) onMessage("Посилання скопійовано!") end
    end
end
...
--=============================================================================
-- 2. ГРАФІЧНИЙ ІНТЕРФЕЙС (Orion Library) з обробкою помилок
--=============================================================================
...
if not success or not OrionLib then
    warn("Не вдалося завантажити Orion Library!")
    onMessage("Помилка завантаження інтерфейсу!")
    return
end

local Window = OrionLib:MakeWindow({
    Name = "Tpa Simulator | ESP & Farm", 
    HidePremium = false, 
    SaveConfig = true, 
    ...
    KeySettings = {
        ...
        Subtitle = "Введіть ключ для доступу",
        Note = "Натисніть 'Get Key', щоб скопіювати посилання",
        FileName = "TpaKey",
        SaveKey = true,
        CustomCheck = function(key) return verifyKey(key) end
    }
})

--=============================================================================
-- 3. ФУНКЦІЇ (ESP та Farm)
--=============================================================================
local MainTab = Window:MakeTab({Name = "Фарм", Icon = "rbxassetid://4483345998"})
local VisualTab = Window:MakeTab({Name = "Візуали", Icon = "rbxassetid://4483345998"})
...
VisualTab:AddToggle({
    Name = "ESP Гравці",
    Default = false,
    Callback = function(v) _G.MASTER_ESP = v end
})

-- Кнопка для ключа у окремій вкладці
local KeyTab = Window:MakeTab({Name = "Ключ", Icon = "rbxassetid://4483345998"})
KeyTab:AddButton({
    Name = "Отримати посилання (Get Key)",
    Callback = function() copyLink() end
})
